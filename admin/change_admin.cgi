...
if check_admin():
	fs=cgi.FieldStorage()
	grant=fs.getfirst('grant')
	cancel=fs.getfirst('cancel')
	adminStatus=check_admin()
	if((grant==adminStatus) or (cancel==adminStatus)):
		errornope()
	else:
		if grant!=None:
			db.users.update_one({'username':grant},{"$set":{"admin":"Y"}})
			print """<script>document.location="../success.php";</script>"""
		
		else:
			db.users.update_one({'username':cancel},{"$set":{"admin":"N"}})			
			print """<script>document.location="../success.php";</script>"""
else:
